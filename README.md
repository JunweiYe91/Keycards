...
- **Card images**: fill in the `Image URL` column in `card_rates.xlsx` with
  either a full `http(s)://` URL, or a local path relative to the project
  root (e.g. `assets/cards/uob_one_card.png`) — the app already ships with
  images for all 8 cards in `assets/cards/`. Leave a row blank and the app
  shows a placeholder image instead. For tiered cards (e.g. UOB One Card's
  3 rows), the same image path is applied to every tier row.
